    ...
    , ui(new Ui::MainWindow)
{
    ...
    //设置窗口的背景图
    QPixmap pixmap("/home/camellia/arm/higher/bg.jpg");
    QPalette palette;
    ...
    //设置边框
    ui->lb_1->setStyleSheet("background:transparent;border:1px solid white;");  //2px表示边框宽度
    ui->lb_2->setStyleSheet("background:transparent;border:1px solid white;");
    ui->lb_3->setStyleSheet("background:transparent;border:1px solid white;");

    //在TCP连接前不可进行上位机其他操作
    ui->pb_R->setDisabled(true);
    ui->pb_Rwarning->setDisabled(true);
    ui->pb_photo->setDisabled(true);
    ui->lb_pic->setStyleSheet("QLabel{background-color:rgb(60,60,60);}");   //图像label背景色在没有显示图像时则为黑色
    ...
    //历史曲线画布初始化
    history_image = QImage(550,360,QImage::Format_RGB32);  //画布的初始化大小设为281*211，使用32位颜色
    fun_history();
...
void MainWindow::fun_displayPhoto(){
    if(localOpened == 0){        //若此时正在浏览本地图片，则动态检测上传的图片先不显示
        QString path = "/home/camellia/arm/higher/img/";
...
void MainWindow::on_pb_open_clicked()
{
    ...
    filters << "*.jpg" << "*.bmp" << "*.png" << "*.jpeg";             //指定只获取扩展名为这四种的图像文件名
...
{
    picNum--;
    if (picNum < 0) picNum = files.size()-1; //若超出第一张图片，则循环跳回最后一张图片
...
{
    picNum++;
    if(picNum >= files.size()) picNum = 0;  //若超出最后一张图片，则循环跳回第一张图片
...
void MainWindow::fun_photo_choose(QString F){    //F为被选中图片的文件名（不包括图片路径）
    if(localOpened == 1){                        //只有在打开了本地图片浏览功能后才能进行下拉菜单选择图片功能
